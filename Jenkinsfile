  ...
  stages {
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep miningpool-middleware | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            TAG=$feature_name DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
          fi
          images=`docker images | grep entropypool | grep miningpool-middleware | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep miningpool-middleware | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          if [ 0 -eq $rc -a x"$revlist" != x ]; then
            tag=`git describe --tags $revlist`
            set +e
            docker images | grep miningpool-middleware | grep $tag
            rc=$?
            set -e
            if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s#currency_proxy: \\\"\\\"#currency_proxy: \\\"$CURRENCY_REQUEST_PROXY\\\"#g" cmd/miningpool-middleware/k8s/00-configmap.yaml
    ...
    stage('Deploy for development') {
      ...
      steps {
        ...
        sh 'sed -i "s#currency_proxy: \\\"\\\"#currency_proxy: \\\"$CURRENCY_REQUEST_PROXY\\\"#g" cmd/miningpool-middleware/k8s/00-configmap.yaml'
